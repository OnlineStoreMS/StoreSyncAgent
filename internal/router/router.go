...
func Setup(h *handler.Handler, cfg *config.Config, webDist string) *gin.Engine {
	...
	adminGroup.Use(adminmw.AdminAuth(&cfg.Auth, jwtMgr))
	{
		...
		adminGroup.GET("/kdzs/accounts/export", h.ExportKdzsAccounts)
		...
		adminGroup.GET("/kdzs/elec-auth", h.ListElecAuth)
		adminGroup.GET("/kdzs/express-templates", h.ListExpressTemplates)
		adminGroup.GET("/kdzs/batch-print-url", h.GetBatchPrintURL)
		adminGroup.POST("/kdzs/print-waybills", h.QueryPrintWaybills)
		adminGroup.GET("/kdzs/shared-accounts", h.ListSharedExpressAccounts)
		...
		adminGroup.POST("/product-stock-alerts/test", h.TestStockAlert)
		adminGroup.POST("/product-stock-alerts/run", h.RunStockAlert)
		adminGroup.POST("/product-stock-alerts/reset-state", h.ResetStockAlertState)
		...
		adminGroup.POST("/orders/cancel-push", h.CancelOrderPush)
		adminGroup.POST("/orders/update-remark", h.UpdateTradeRemark)
		adminGroup.POST("/orders/ship-callback", h.ShipCallback)
		adminGroup.POST("/orders/hand/parse-address", h.ParseHandAddress)
		adminGroup.POST("/orders/hand/create", h.CreateHandOrder)
		adminGroup.POST("/orders/hand/batch-create", h.BatchCreateHandOrder)
