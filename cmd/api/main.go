...
func main() {
	...
	stockAlertRepo := repo.NewStockAlert(db)
	mgr := service.NewManager(cfg, kdzsRepo, returnExchangeRepo, notificationRepo, stockAlertRepo)
	h := handler.New(mgr)
	notifyScheduler := scheduler.NewNotificationScheduler(mgr)
	notifyScheduler.Start()
	stockAlertScheduler := scheduler.NewStockAlertScheduler(mgr)
